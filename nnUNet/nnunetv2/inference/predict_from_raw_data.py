...
from nnunetv2.utilities.helpers import empty_cache, dummy_context, load_nnunet_checkpoint
...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            checkpoint = load_nnunet_checkpoint(
    ...
    def _network_module_for_export(self) -> nn.Module:
        """Underlying nn.Module for export (unwrap torch.compile / DDP)."""
        net = self.network
    ...
    def export_to_jit_and_onnx(self,
                               output_folder: str,
                               fold_indices: Optional[List[int]] = None,
                               opset_version: int = 17,
                               export_jit: bool = True,
                               export_onnx: bool = True,
                               configuration_name: Optional[str] = None) -> List[str]:
        """
        Export per-fold network weights to TorchScript and ONNX.

        Reuses the same architecture and checkpoints as
        ``initialize_from_trained_model_folder``. Exports a single patch forward
        (logits), not sliding-window inference, TTA, or segmentation postprocessing.
        """
        if self.network is None or self.list_of_parameters is None:
            raise RuntimeError(
                'Predictor not initialized. Call initialize_from_trained_model_folder first.')

        if not export_jit and not export_onnx:
            raise ValueError('At least one of export_jit or export_onnx must be True.')
        ...
        dummy_input = torch.randn(
            1, num_input_channels, *patch_size, dtype=torch.float32, device=torch.device('cpu'))

        if fold_indices is None:
            fold_indices = list(range(len(self.list_of_parameters)))

        network = self._network_module_for_export()
        network.eval()
        network.cpu()

        spatial_names = ['d', 'h', 'w'] if len(patch_size) == 3 else ['h', 'w']
        dynamic_axes = {
            'input': {0: 'batch'},
            'logits': {0: 'batch'},
        }
        for i, name in enumerate(spatial_names):
            dynamic_axes['input'][2 + i] = name
            dynamic_axes['logits'][2 + i] = name

        exported_files: List[str] = []
        multi_fold = len(self.list_of_parameters) > 1

        for fold_idx in fold_indices:
            if fold_idx < 0 or fold_idx >= len(self.list_of_parameters):
                raise ValueError(
                    f'fold index {fold_idx} out of range (n_folds={len(self.list_of_parameters)})')

            network.load_state_dict(self.list_of_parameters[fold_idx])
            suffix = f'_fold_{fold_idx}' if multi_fold else ''

            with torch.no_grad():
                if export_jit:
                    traced = torch.jit.trace(network, dummy_input, strict=False)
                    jit_path = join(output_folder, f'model{suffix}.pt')
                    traced.save(jit_path)
                    exported_files.append(jit_path)
                    print(f'Saved TorchScript: {jit_path}')

                if export_onnx:
                    onnx_path = join(output_folder, f'model{suffix}.onnx')
                    torch.onnx.export(
                        network,
                        dummy_input,
                        onnx_path,
        ...
        export_meta = {
            ...
            'inference_allowed_mirroring_axes': list(self.allowed_mirroring_axes)
            if self.allowed_mirroring_axes is not None else None,
            'exported_folds': fold_indices,
            'files': exported_files,
            'note': (
                'ONNX/JIT export is patch-level forward only. '
                'Full nnU-Net inference still requires preprocessing, sliding window, '
                'and convert_predicted_logits_to_segmentation_with_correct_shape. '
                'Use nnUNetOnnxPredictor.initialize_from_onnx_folder for ONNX inference.'
            ),
        }
        meta_path = join(output_folder, 'export_metadata.json')
        save_json(export_meta, meta_path)
...
def export_entry_point_modelfolder():
    """
    CLI mirror of nnUNetv2_predict_from_modelfolder, but only exports JIT/ONNX.

    Example (same -m / -f / -chk as predict, no input images needed)::

        nnUNetv2_export_from_modelfolder -m configs -o model_export -f 0 -chk checkpoint_best.pth
    """
    import argparse
    parser = argparse.ArgumentParser(
        description='Export nnU-Net trained model to TorchScript (.pt) and ONNX (.onnx). '
                    'Uses the same model folder / fold / checkpoint arguments as '
                    'nnUNetv2_predict_from_modelfolder.')
    parser.add_argument('-m', type=str, required=True,
                        help='Folder containing the trained model (fold_X subfolders).')
    parser.add_argument('-o', type=str, required=True,
                        help='Output folder for exported .pt / .onnx files.')
    parser.add_argument('-f', nargs='+', type=str, required=False, default=(0,),
                        help='Fold(s) to export. Default: (0,)')
    parser.add_argument('-chk', type=str, required=False, default='checkpoint_final.pth',
                        help='Checkpoint file name inside each fold folder. Default: checkpoint_final.pth')
    parser.add_argument('-opset-version', '--opset-version', '-opset', type=int, required=False, default=17,
                        help='ONNX opset version. Default: 17. Use -opset-version 20 or --opset-version 20.')
    ...
    # Export on CPU; do not enable torch.compile (handled inside initialize + unwrap for export).
    old_compile = os.environ.pop('nnUNet_compile', None)
    try:
        ...
        fold_indices = list(range(len(predictor.list_of_parameters)))
        from nnunetv2.inference.onnx_predictor import nnUNetOnnxPredictor
        configuration_name = nnUNetOnnxPredictor.configuration_name_from_model_folder(args.m)
        predictor.export_to_jit_and_onnx(
            args.o,
...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        Path('/home/ZHSun/szh_space/workspace/spine-segmentation-nnunet/nnUNetTrainer_250epochs__nnUNetPlans__3d_fullres'),
        use_folds=(0,),
    ...
    img, props = SimpleITKIO().read_images(['imagesTs/SPINE_003_0000.nii.gz'])
    ret = predictor.predict_single_npy_array(img, props, None, None, False)

    iterator = predictor.get_data_iterator_from_raw_npy_data([img], None, [props], None, 1)
    ret = predictor.predict_from_data_iterator(iterator, False, 1)

    # --- ONNX inference (export once, then predict with onnxruntime) ---
    # pip install onnxruntime   # or onnxruntime-gpu for CUDA
    # onnx_export_dir = join(nnUNet_results, 'Dataset003_Liver/onnx_export')
    # predictor.export_to_jit_and_onnx(
    #     onnx_export_dir,
    #     fold_indices=[0],
    #     export_jit=False,
    #     export_onnx=True,
    #     configuration_name=nnUNetOnnxPredictor.configuration_name_from_model_folder(
    #         join(nnUNet_results, 'Dataset003_Liver/nnUNetTrainer__nnUNetPlans__3d_lowres')),
    # )
    # from nnunetv2.inference.onnx_predictor import nnUNetOnnxPredictor
    # onnx_predictor = nnUNetOnnxPredictor(
    #     tile_step_size=0.5,
    #     use_gaussian=True,
    #     use_mirroring=True,
    #     perform_everything_on_device=True,
    ...
    #     allow_tqdm=True,
    # )
    # model_dir = join(nnUNet_results, 'Dataset003_Liver/nnUNetTrainer__nnUNetPlans__3d_lowres')
    # onnx_predictor.initialize_from_onnx_folder(onnx_export_dir, model_dir, use_folds=(0,))
    # onnx_predictor.predict_from_files(
    #     join(nnUNet_raw, 'Dataset003_Liver/imagesTs'),
    #     join(nnUNet_raw, 'Dataset003_Liver/imagesTs_pred_onnx'),
    #     save_probabilities=False, overwrite=False,
    #     num_processes_preprocessing=2, num_processes_segmentation_export=2,
    #     folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0,
    # )
